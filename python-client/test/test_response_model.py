# coding: utf-8

"""
    OmicIDX

        The OmicIDX API documentation is available in three forms:  - [RapiDoc](/docs) - [OpenAPI/Swagger Interactive](/swaggerdoc) - [ReDoc (more readable in some ways)](/redoc)    # noqa: E501

    The version of the OpenAPI document: 0.99
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import omicidx_client
from omicidx_client.models.response_model import ResponseModel  # noqa: E501
from omicidx_client.rest import ApiException

class TestResponseModel(unittest.TestCase):
    """ResponseModel unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test ResponseModel
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = omicidx_client.models.response_model.ResponseModel()  # noqa: E501
        if include_optional :
            return ResponseModel(
                hits = [
                    None
                    ], 
                facets = [
                    None
                    ], 
                stats = omicidx_client.models.response_stats.ResponseStats(
                    total = 56, 
                    took = 1.337, ), 
                success = True, 
                cursor = '0'
            )
        else :
            return ResponseModel(
                hits = [
                    None
                    ],
                facets = [
                    None
                    ],
                stats = omicidx_client.models.response_stats.ResponseStats(
                    total = 56, 
                    took = 1.337, ),
                success = True,
        )

    def testResponseModel(self):
        """Test ResponseModel"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
